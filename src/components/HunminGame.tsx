...
export const HunminGame: React.FC = () => {
    ...
    // Use teams directly, but filtering for connection might be good visually, or just show all.
    // User asked "1들이라고 앞에 나오는게 아니라 위치를 고정", implies fixed order. 
    // gameState.teams comes from Object.values(teams), which is generally stable by insertion order.
    // We will NOT sort them.
    ...
    return (
        <div className="min-h-screen bg-paper-100 p-4 md:p-8 font-sans flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-1 flex flex-col items-center justify-center min-h-[50vh] md:min-h-0">
                ...
                <div className="inline-block px-4 py-1 bg-accent-secondary text-white rounded-full text-sm font-bold mb-8 shadow-sm animate-fade-in uppercase tracking-wider">
                    훈민정음 퀴즈 ({currentQuizIndex + 1} / {totalQuizCount})
                </div>

                {/* 3. Central Prompt: Larger and cleaner */}
                <h1 className="text-7xl md:text-[10rem] font-black text-paper-900 tracking-tighter drop-shadow-sm mb-8 text-center leading-none select-none">
